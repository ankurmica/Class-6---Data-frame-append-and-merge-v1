...
month <- rep("Feb",nrow(spend.feb14))
...
cust_spend <-merge(cust,
                   spend,
                   by="ID") # Intersection
# Left, will have records from all ids present in left side
cust_spend_left <-merge(cust,
                        spend,
                        by="ID",
                        all.x=TRUE) #all.x here is first data
# Right, will have records from all ids present in right side
